...
def load_config(config_path: str | None) -> dict:
    ...
    3) If not found:
         fall back to the default 'config.toml' embedded in the oris package.
    ...
    try:
        pkg_config = importlib_resources.files("oris") / "config.toml"
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="oris",
        description="ORIS pipeline launcher for Slurm HPC.",
    )
...
def clean_previous_results(zips_dir: str, verbose: bool = True) -> None:
    """
    Remove previous ORIS result files from each zip in `zips_dir`.
...
def main(argv: list[str] | None = None) -> None:
    ...
    if in_slurm:
        print("[WARN] oris_cli is designed to be run from the login node, not inside a Slurm job.")
    ...
    # 6a) Clean previous ORIS results from the zip files
    print("[INFO] Cleaning previous ORIS results (Results/ folder) in ZIPs...")
    ...
    run_root = zips_path.parent / f"oris_run_{timestamp}"
    ...
    print(f"[INFO] ORIS run directory: {run_root}")
    ...
    out_pattern = slurm_run_cfg.get("output") or "oris_%j.out"
    err_pattern = slurm_run_cfg.get("error") or "oris_%j.err"
    ...
    if sampling and sampling > 0:
        ...
        sampling_script_name = f"oris_sampling_{timestamp}.sbatch"
    ...
    if args.mode == "synergies":
        pipeline_body = textwrap.dedent(
            ...
            mpirun python3 -m oris.compute_synergies "$zip_dir"
        ...
        pipeline_body = textwrap.dedent(
            ...
            mpirun python3 -m oris.count_allpaths "$zip_dir"
    ...
    pipeline_script_name = f"oris_{args.mode}_{timestamp}.sbatch"
